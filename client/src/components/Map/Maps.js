	...
	useEffect(() => {
		const getJson = async () => {
			try {
				...
				const endHikePoint = point.filter(
					(p) => p.type === "arrive"
				)[0];
				const others = point
			...
			{coordinates.length && (
				<center>
					...
					>
						...
						<Marker
							position={arrive}
							icon={GetCustomIcon("ending")}
						>
							<Popup>Starting Point</Popup>
						</Marker>

						{/*Starting Point Path*/}
						{start[0] !== firstPoint[0] &&
							start[1] !== firstPoint[1] && (
								<Marker position={firstPoint}></Marker>
							)}

						{/*Ending Point Path*/}
						{arrive[0] !== lastPoint[0] &&
							arrive[1] !== lastPoint[1] && (
	...
	return (
		<Modal size="lg" show={showModal} onHide={() => setShowModal(false)}>
			...
			<Modal.Body>
				{areadragmap && (
					<div className="mb-3">
						Press ctrl + mouse drag to define a geographic area
						where you want to find hikes
					</div>
				)}
