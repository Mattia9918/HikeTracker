...
router.post(
  "/api/hutLinkHike",
  checkAuth.isLocalGuide,
  [check("hikeid").isInt(), check("pointid").isInt()],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).json({ errors: errors.array() });
    ...
    try {
      ...
      if (hike.localguideID !== req.user.id) {
        return res.status(403).json({
          error: "Operation forbidden: you must be creator of the hike",
        });
      }
    ...
    filters.forEach(async (filter) => {
      switch (filter.filterName) {
        ...
        case "altitude":
          huts = huts.filter(
            (hut) =>
              hut.altitude >= filter.value1 && hut.altitude <= filter.value2
          );
          break;

        case "restaurant_service":
          huts = huts.filter((hut) => hut.restaurant_service == 1);
          break;
        case "disabled_services":
          huts = huts.filter((hut) => hut.disabled_services == 1);
          break;
        case "bike_friendly":
          huts = huts.filter((hut) => hut.bike_friendly == 1);
          break;
        case "city":
          huts = huts.filter((hut) => hut.city == filter.value1);
          break;
        case "province":
          huts = huts.filter((hut) => hut.province == filter.value1);
          break;
        case "beds":
          huts = huts.filter((hut) => hut.beds > 0);
          break;
        case "reachability":
          huts = huts.filter((hut) => hut.reachability == filter.value1);
          break;
        case "area":
...
router.get("/api/hutsDistantFromHike/:hikeId", async (req, res) => {
  try {
    ...
    const filteredHuts = await functions.getItemDistantFromHike(
      req.params.hikeId,
      huts,
      5
    );
    ...
    else res.status(200).json(filteredHuts);
  } catch (err) {
    console.log(err);
    res.status(500).end();
  }
