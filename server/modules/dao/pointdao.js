...
exports.getPointByHikeId = (id) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT latitude,longitude,H.type FROM hike_point H,point P WHERE H.hikeID=? AND H.pointID=P.id ";
...
exports.getHutMap = () => {
  return new Promise((resolve, reject) => {
    const sql = "SELECT latitude,longitude FROM point P WHERE type='hut'";
    db.all(sql, [], (err, rows) => {
      if (err) reject(err);
      else resolve(rows);
    });
...
exports.getParkingMap = () => {
  return new Promise((resolve, reject) => {
    const sql = "SELECT latitude,longitude FROM point P WHERE type='parking'";
    db.all(sql, [], (err, rows) => {
      if (err) reject(err);
      else resolve(rows);
    });
